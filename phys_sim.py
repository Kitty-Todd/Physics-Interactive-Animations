...
class Friction(Scene):
    def construct(self):
        params = {"mass": 1.0, "gravity": 9.8, "static_friction": 0.5, "kinetic_friction": 0.3,"max_force": 10.0}
        ...
        m = params["mass"]
        g = params["gravity"]
        s = params["static_friction"]
        k = params["kinetic_friction"]
        F_app = params["max_force"]
        ...
        a = Arrow(start = b.get_right(), end = RIGHT*(F_app+3.1) + DOWN * 1.45, color = BLUE, stroke_width = 4) #block applied force start!!
        aa = Arrow(start = b.get_right(), end = RIGHT*(F_app+3.1) + DOWN * 1.45, color = BLUE, stroke_width = 4) #block applied force end!!
        a1 = Arrow(start = d.get_right(), end = LEFT*(3.5-F_app/7) + UP*1.5, color = BLUE, stroke_width = 4) #dot applied force start!!
        a1a = Arrow(start = d.get_right(), end = LEFT*(3.5-F_app/7) + UP*1.5, color = BLUE, stroke_width = 4) #dot applied force end!!
        ...
        a2 = Arrow(start = b.get_left(), end = LEFT*(fs+3.1)+ DOWN*1.45, color = RED, stroke_width = 4) #block static friction!!
        a2a = Arrow(start = b.get_left(), end = LEFT*(fk+3.1) + DOWN*1.45, color = RED, stroke_width = 4) #block kinetic friction!!
